...
if (NODE_ENV === 'production') {
	if (!GCS_BUCKET_NAME || !GCP_PROJECT_ID) {
		console.error(
			'[FileStorage] FATAL ERROR: GCS_BUCKET_NAME and GCP_PROJECT_ID must be set in production environment.'
		);
		...
		try {
			...
			console.error(
				'[FileStorage] FATAL ERROR: Could not initialize Google Cloud Storage client.',
				error
			);
			storage = null;
		}
