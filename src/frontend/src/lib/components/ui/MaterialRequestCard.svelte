...
<script lang="ts">
	...
	interface Props {
		...
		showInterestButton?: boolean; // Default to false
		currentUserId?: string | null | undefined;
		onSendInterest?: (detail: { customerId: string; materialRequestId: string }) => void;
	}

	let { request, showInterestButton = false, currentUserId = null, onSendInterest }: Props = $props();



	let alreadyInterested = $derived(!!(currentUserId && request.interestedSuppliers?.includes(currentUserId)));
