...
<div class="flex min-w-[250px] items-center gap-2 p-1">
	<button
		on:click={togglePlay}
		disabled={isLoading || !!errorMessage}
		class="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-emerald-600 text-white transition-colors hover:bg-emerald-500 disabled:cursor-not-allowed disabled:bg-slate-600"
	...
	<div class="w-full flex-1">
		...
		{:else}
			<div bind:this={waveformEl} class="h-10 w-full"></div>
		{/if}
	</div>

	<div class="w-fit text-right font-mono text-xs text-slate-400">
		{#if !isLoading && !errorMessage}
			<span
				>{#if isPlaying}{currentTime}{:else}{totalDuration}{/if}</span
			>
		{/if}
	</div>
</div>
